...
#define COLOR_RAMP(colors, factor, finalColor) \
  ...
  for (int i = 0; i < 2; i++) {                               \
    vec3 col = colors[i];                                    \
    bool isInBetween = float(i) * 0.5 <= factor;            \
    index = isInBetween ? i : index;                        \
  }                                                         \
  vec3 currentColor = colors[index];                        \
  vec3 nextColor = colors[index + 1];                       \
  float range = (float(index + 1) * 0.5) - (float(index) * 0.5); \
  float lerpFactor = (factor - float(index) * 0.5) / range; \
...
void main() {
  ...
  vec3 rampColor;
  COLOR_RAMP(uColorStops, uv.x, rampColor);
...
export default function Aurora({ colorStops = ['#6366f1', '#8b5cf6', '#06b6d4'], amplitude = 1.0, blend = 0.5, speed = 1.0 }) {
